...
const detectLanguagesSelect = document.getElementById("cmt-default-detect-language"); 
const translateLanguagesSelect = document.getElementById("cmt-default-translate-language"); 

var defaultIntialLanguage, defaultTranslateLanguage;
...
if(defaultIntialLanguage == 'auto'){
    default_option.selected = 'selected'
}
detectLanguagesSelect.appendChild(default_option);

for (var iso_code in iso_codes) {
    if (iso_codes.hasOwnProperty(iso_code)) {
        var option_detect = document.createElement('option');
        if(iso_code == defaultIntialLanguage){
            option_detect.selected = 'selected'
        }
        option_detect.value = iso_code;
        var iso_code_name = iso_codes[iso_code].name;
        console.log(iso_code_name)
        option_detect.title = iso_code_name
        iso_code_name = (iso_code_name.length > 17) ? iso_code_name.substring(0, 17) + "..." : iso_code_name;
        option_detect.innerHTML = iso_code_name;
        var option_translate = option_detect.cloneNode();
...
detectLanguagesSelect.addEventListener('change', (event) => {
    chrome.storage.local.set({
        defaultIntialLanguage: event.target.value
    });
});

translateLanguagesSelect.addEventListener('change', (event) => {
    chrome.storage.local.set({
